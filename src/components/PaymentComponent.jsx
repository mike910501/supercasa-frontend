...
paquetes: carrito.filter(item => item.tipo === 'paquete').map(item => ({
  ...
  // ===================================
  // 🔄 NUEVA FUNCIÓN: LONG POLLING MEJORADO
  // ===================================
  const esperarConfirmacionMejorada = async (txId) => {
    console.log(`⏳ Esperando confirmación mejorada para: ${txId}`);
    
    try {
      const token = localStorage.getItem('token');
      
      // Crear un AbortController para timeout del cliente
      ...
      console.log('📊 Respuesta long polling:', resultado);
      
      if (resultado.success && resultado.status === 'APPROVED') {
        console.log('🎉 ¡Pago confirmado vía long polling!');
        setEstadoPago('completado');
        setProcesando(false);
        
        // Dar tiempo para que todo se actualice
        setTimeout(() => {
          onPaymentSuccess({
            ...resultado,
        ...
        console.log('⏰ Timeout del servidor, iniciando polling tradicional como backup');
        // Fallback al polling tradicional
      ...
      if (error.name === 'AbortError') {
        console.log('⏰ Timeout del cliente, usando polling tradicional');
      }
      
      // Si falla, usar polling tradicional como backup
      iniciarVerificacionPago(txId);
      return false;
    }
  };
